...
export function createXMIElement(type, name, attributes = [], operations = [], specialisations = [], aggregations = []) {
    ...
    const formattedName = removeBlockType(name).replace(/[^a-zA-Z0-9_]/g, ''); // Rimuove i caratteri non validi
    ...
    attributes.forEach(attribute => {
        ...
        element_code += `\t\t\t\t<ownedAttribute xmi:id="${attribute_id}" name="${attribute.name.toLowerCase()}" xmi:type="uml:Property"/>\n`;
    });

    // Aggiungi operazioni
    operations.forEach(operation => {
        const operation_id = generateID(operation.name);
        const formattedOperationName = removeBlockType(operation.name).replace(/[^a-zA-Z0-9_]/g, ''); // Rimuove i caratteri non validi dall'operazione
        ...
        if (operation.target !== "NONE" && operation.target != undefined) {
            ...
            const formattedTarget = removeBlockType(operation.target).replace(/[^a-zA-Z0-9_]/g, ''); // Rimuove i caratteri non validi dal target
    ...
    aggregations.forEach(agg => {
        if (agg !== "NONE") {
            ...
            const member_end_1_id = generateRandID();
            const member_end_2_id = generateRandID();
            const formattedAgg = removeBlockType(agg).replace(/[^a-zA-Z0-9_]/g, ''); // Rimuove i caratteri non validi dall'aggregazione
    ...
    specialisations.forEach(specialisation => {
        ...
        const formattedSpecialisation = removeBlockType(specialisation.name).replace(/[^a-zA-Z0-9_]/g, ''); // Rimuove i caratteri non validi dalla specializzazione
